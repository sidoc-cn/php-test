...
class MiddlewareTool {
    ...
    static public function getToken($request) {
        ...
        // 0.1> 从请求头中获取token（分别兼容从请求头、Cookie和请求参数中来获取token）
        $token = $request->header("authorization");
        if(!empty($token)){
            self::configCookie($token); // 配置Cookie以兼容未能前后分离的项目
            return $token;
        }
    ...
    static private function configCookie($token) {
        ...
        $expires = time() + 604800 + ((24 - $hour + 4) * 3600); // 有效时长(秒)，此处为一周(604800秒)，一周后凌晨4点过期
