...
class DomainTool{
     ...
     * 查询域名备案信息
     * 
     * 注：此法绕过工信部官网的行为验证来查询备案信息，并不可靠；一旦工信修改行为验证相关算法，此法将失效；
     *
     * @param [type] $domain
     * @return void
     */
    static public function filingInquiry($domain){

        $timeStamp = time();
        $authKey = md5("testtest" . $timeStamp);
        $token = json_decode(self::curl_post("auth", "authKey=$authKey&timeStamp=$timeStamp", "application/x-www-form-urlencoded;charset=UTF-8", "0"));
        $token = $token->params->bussiness;
        $query = json_decode(self::curl_post("icpAbbreviateInfo/queryByCondition", '{"pageNum":"","pageSize":"","unitName":"' . $domain . '"}', "application/json;charset=UTF-8", $token));
    ...
    static public function curl_post($url, $data, $Content, $token) {
    ...
    static public function getTopHost($url) {
